...
<?php
   ...
   /* WS5 Wholesale - check for wholesale customer + wholesale price if exist  */
    if ( (tep_session_is_registered('wholesale_customers_id')) && ($wholesale_price = tep_get_products_wholesale_price($product_info['products_id'])) ) {
      $products_price = '<span class="productWholesalePrice">' . $currencies->display_price($wholesale_price, tep_get_tax_rate(MODULE_CONTENT_WHOLESALE_LOGIN_FORM_TAX)) . '</span><br><small>' .
	  tep_get_products_wholesale_note($product_info['products_id']) . '</small>';
    } else {
