    ...
    while ($listing = tep_db_fetch_array($listing_query)) {
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        switch ($column_list[$col]) {
          ...
          if ( (tep_session_is_registered('wholesale_customers_id')) && ($wholesale_price =      tep_get_products_wholesale_price($listing['products_id'])) ) {
          $prod_list_contents .=  '<td align="right"><span class="productWholesalePrice">' . $currencies->display_price($wholesale_price, tep_get_tax_rate(MODULE_CONTENT_WHOLESALE_LOGIN_FORM_TAX)) . '</span></td>';
    } else {
		   /* original */
