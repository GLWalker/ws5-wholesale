...
// products price
        ...
        if ($product = tep_db_fetch_array($product_query)) {
		  ...
		  /* WS5 Wholesale - grab wholesale prices and weight */
		  if (tep_session_is_registered('wholesale_customers_id')) {
		   $wholesale_query = tep_db_query("select wholesale_price, wholesale_weight from " . TABLE_PRODUCTS_TO_WHOLESALE . " where products_id = '" . (int)$prid . "'");
          if (tep_db_num_rows ($wholesale_query)) {
            $wholesale = tep_db_fetch_array($wholesale_query);
            $products_price = $wholesale['wholesale_price'];
			if (tep_not_null($wholesale['wholesale_weight'])) {
			  $products_weight = $wholesale['wholesale_weight'];
			}
			$products_tax = tep_get_tax_rate(MODULE_CONTENT_WHOLESALE_LOGIN_FORM_TAX);
          }
		  /* end WS5 Wholesale */
		  }
    ...
    function get_products() {
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        if ($products = tep_db_fetch_array($products_query)) {
		 ...
		 /* WS5 Wholesale - Changes for new weight and tax */
		  $products_tax = tep_get_tax_rate($product['products_tax_class_id']);
          $products_weight = $product['products_weight'];
		  /* end WS5 Wholesale */
		...
		if (tep_session_is_registered('wholesale_customers_id')) {
          ...
          if (tep_db_num_rows ($wholesale_query)) {
			...
			$products_tax = MODULE_CONTENT_WHOLESALE_LOGIN_FORM_TAX;
          }
		  /* end WS5 Wholesale */
		}
          ...
          $products_array[] = array('id' => $products_id,
									...
									/* WS5 Wholesale - push wholesale weight */
                                    'weight' => $products_weight,
									/* end WS5 Wholesale */
                                    'final_price' => ($products_price + $this->attributes_price($products_id)),
									/* WS5 Wholesale - push wholesale tax */
                                   // 'tax_class_id' => $products['products_tax_class_id'],
									'tax_class_id' => $products_tax,
									/* end WS5 Wholesale */
