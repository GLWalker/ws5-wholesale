...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  ...
  /* WS5 Wholesale - Return a product's wholesale price if exist */
  function tep_get_products_wholesale_price($product_id) {
    $product_query = tep_db_query("select wholesale_price from " . TABLE_PRODUCTS_TO_WHOLESALE . " where products_id = '" . (int)$product_id . "'");
    $product = tep_db_fetch_array($product_query);
    return $product['wholesale_price'];
  }
  function tep_get_products_wholesale_weight($product_id) {
    $product_query = tep_db_query("select wholesale_weight from " . TABLE_PRODUCTS_TO_WHOLESALE . " where products_id = '" . (int)$product_id . "' and wholesale_weight is not null");
    $product = tep_db_fetch_array($product_query);
    return $product['wholesale_weight'];
  }
  function tep_get_products_wholesale_note($product_id) {
    $product_query = tep_db_query("select wholesale_note from " . TABLE_PRODUCTS_TO_WHOLESALE . " where products_id = '" . (int)$product_id . "' and wholesale_note is not null");
    $product = tep_db_fetch_array($product_query);
    return $product['wholesale_note'];
  }
   /* end WS5 Wholesale */
?>
