...
/*
  ...
  Modified June 2014 for addition of 
  WS5 Wholesale Addon
...
*/
  ...
  /* WS5 Wholesale - placing function to set customer wholesale status here as we only need in the customers.php @ this time. If your picky, then move it to the includes/functions/general.php ~ or even better, create a addon_functions.php and call it in application_top.php and start adding all your misc funtions from various addons in there */

  // also doing the same with database table
  define('TABLE_CUSTOMERS_TO_WHOLESALE', 'customers_to_wholesale');
  
  function tep_set_wholesale_status($customers_id, $status) {
    if ($status == '1') {
      return tep_db_query("insert into " . TABLE_CUSTOMERS_TO_WHOLESALE . " (customers_id) values ('" . (int) $customers_id . "')");
    } elseif ($status == '0') {
      return tep_db_query("delete from " . TABLE_CUSTOMERS_TO_WHOLESALE . " where customers_id = '" . (int)$customers_id . "'");
    } else {
      return -1;
    }
  }
  ...
  if (tep_not_null($action)) {
    switch ($action) {
	 /* WS5 Wholesale - setflag as seen previously in categories.php*/
	  case 'setflag':  
        if ( ($HTTP_GET_VARS['flag'] == '0') || ($HTTP_GET_VARS['flag'] == '1') ) {
          if (isset($HTTP_GET_VARS['cID'])) {
            tep_set_wholesale_status($HTTP_GET_VARS['cID'], $HTTP_GET_VARS['flag']);
          }
        } 
        tep_redirect(tep_href_link(FILENAME_CUSTOMERS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $HTTP_GET_VARS['cID']));
        break;
	 /* end WS5 Wholesale */
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <!--/* WS5 Wholesale - new table heading cell */-->
...
<?php
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
	  ...
	  /* WS5 Wholesale - grab the wholesale customer info */
	  $wholesale_query = tep_db_query("select customers_id from " . TABLE_CUSTOMERS_TO_WHOLESALE . " where customers_id = '" . (int)$customers['customers_id'] . "'");
      $wholesale = tep_db_fetch_array($wholesale_query);
	  /* end WS5 Wholesale */
      ...
      if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) {
	   /* WS5 Wholesale - need to remove the row onclick so we can set wholesale status  
...
?>
                ...
                <!--/* WS5 Wholesale - new table body cell with controls setting wholesale status of customer */-->
                <td class="dataTableContent" align="right">
<?php
      if  ($wholesale['customers_id'] == $customers['customers_id'] ) {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CUSTOMERS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $customers['customers_id'] . '&action=setflag&flag=0') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $customers['customers_id'] . '&action=setflag&flag=1') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?></td>
               <!--/* end WS5 Wholesale */-->
